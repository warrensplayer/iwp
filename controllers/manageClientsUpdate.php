				...
				elseif($PTC == 'plugins' || $PTC == 'themes'){
					...
					foreach($PTCResponse['upgraded'] as $name => $success){
						...
						elseif(!empty($success)){
							$status = 'error';
							DB::update("?:history_additional_data", array('status' => $status, 'errorMsg' => $success), "historyID=".$historyID." AND (uniqueName='".$name."' OR MD5(uniqueName)='".$name."') AND detailedAction='".$itemType."'");
						}
						else{
							$status = 'error';
							DB::update("?:history_additional_data", array('status' => $status, 'error' => 'unknown', 'errorMsg' => 'An Unknow error occured.'), "historyID=".$historyID." AND (uniqueName='".$name."' OR MD5(uniqueName)='".$name."') AND detailedAction='".$itemType."'");
						}
					}
				}
			}
